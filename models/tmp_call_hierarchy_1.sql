...
LEFT JOIN {{ var('schema') }}.account_business as ACC ON a.account_id = ACC.id
LEFT JOIN {{ var('schema') }}.users_business AS USR ON USR.id = a.aux_owner_id
LEFT JOIN {{ var('schema') }}.country_settings_business cs on A.Country_Code = cs.jj_Country_ISO_Code__c
LEFT JOIN {{ var('schema') }}.country_settings_business cs2 ON USR.jj_user_country__C = cs2.name
