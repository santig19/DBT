...
LEFT JOIN {{ source('raw', 'user') }} us on c.OwnerId = us.Id
LEFT JOIN {{ source('raw', 'account') }} a on c.Account_vod__c = a.Id
LEFT JOIN {{ source('raw', 'record_type') }} rt on c.RecordTypeId = rt.Id
LEFT JOIN (
	SELECT ROW_NUMBER() OVER (PARTITION BY IsoCode ORDER BY lastmodifieddate DESC) as rowid,IsoCode 
	FROM {{ source('raw', 'currency_type') }}
...
LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY parentid ORDER BY lastmodifieddate DESC) as rowid,parentid,value,Language 
	FROM {{ source('raw', 'record_type_localization') }} 
