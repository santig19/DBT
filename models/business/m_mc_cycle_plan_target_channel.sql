...
(
  select distinct
         ...
         cast(case when ifnull(cpchannel.Channel_Activity_Actual_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Actual_vod__c end as float8) as mc_cycle_plan_channel_activity_actual,
         cast(case when ifnull(cpchannel.Channel_Activity_Goal_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Goal_vod__c end as float8) as mc_cycle_plan_channel_activity_goal,
         cast(case when ifnull(cpchannel.Channel_Activity_Remaining_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Remaining_vod__c end as float8) as mc_cycle_plan_channel_activity_remaining,

         case 
           when (cast(case when ifnull(cpchannel.Channel_Activity_Actual_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Actual_vod__c end as float8)) 
              <= (cast(case when ifnull(cpchannel.Channel_Activity_Goal_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Goal_vod__c end as float8))
             then (cast(case when ifnull(cpchannel.Channel_Activity_Actual_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Actual_vod__c end as float8)) 
           else (cast(case when ifnull(cpchannel.Channel_Activity_Goal_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Goal_vod__c end as float8))
         end as mc_cycle_plan_channel_activity_valid,

         ifnull(cpchannel.Channel_vod__c,'') as mc_cycle_plan_channel,
         ifnull(cpchannel.id,'') as mc_cycle_plan_channel_id,
         ifnull(cpchannel.Cycle_Channel_vod__c,'') as mc_cycle_channel_id,
         cast(case when ifnull(cpchannel.Channel_Activity_Max_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Max_vod__c end as float8) as mc_cycle_plan_channel_activity_max,

         case
           when (cast(case when ifnull(cpchannel.Channel_Activity_Actual_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Actual_vod__c end as float8)) 
               > (cast(case when ifnull(cpchannel.Channel_Activity_Max_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Max_vod__c end as float8))
             then (cast(case when ifnull(cpchannel.Channel_Activity_Max_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Max_vod__c end as float8))
           else (cast(case when ifnull(cpchannel.Channel_Activity_Actual_vod__c,'') = '' then '0' else cpchannel.Channel_Activity_Actual_vod__c end as float8))
         end as mc_cycle_plan_channel_activity_actual_compared_to_max,

         cast(case when ifnull(cchannel.Channel_Weight_vod__c,'') = '' then '0' else cchannel.Channel_Weight_vod__c end as float8) as mc_cycle_channel_weight
